...
export const getOffersByLocationFunc = functions
  ...
  .https.onCall(monitorFunction('getOffersByLocation', async (data: GetOffersByLocationRequest, context) => {
    const { toOfferDTO } = await import('./adapters/offerDto');
    const result = await getOffersByLocation(data, { db });
    
    // Convert core offers to DTO format for Flutter
    if (result.success && result.offers) {
      const mappedOffers = result.offers.map((offer: any) => {
        const raw = {
          id: offer.offerId || offer.id,
          title: offer.title,
          description: offer.description,
          points_value: offer.pointsValue,
          points_required: offer.pointsValue,
          points_cost: offer.pointsValue,
          valid_until: offer.validUntil,
          created_at: offer.validFrom,
          merchant_id: offer.merchantId,
          merchant_name: offer.merchantName || '',
          category: offer.category,
          distance_km: offer.distance,
          is_active: offer.status === 'active',
          image_url: '',
          discount_percentage: 0
        };
        return toOfferDTO(raw, false);
      });
      
      return {
        success: true,
        offers: mappedOffers as any,  // Return DTO format instead of OfferWithDistance
...
export const getBalance = functions
  ...
  .https.onCall(monitorFunction('getBalance', async (data, context) => {
    // RC Contract: Use auth.uid if customerId not provided
    if (!context.auth) {
      throw new functions.https.HttpsError('unauthenticated', 'Authentication required');
    }
    
    const customerId = data.customerId || context.auth.uid;
    
    const result = await getPointsBalance({ ...data, customerId }, context, { db });
    
    // Ensure both 'balance' and 'totalBalance' are present for Flutter
    if (result && typeof result.totalBalance === 'number') {
      return {
        ...result,
        balance: result.totalBalance  // Add balance alias for Flutter
      };
    }
