...
# Mobile Merchant Integration Proof Script
# Tests mobile-merchant with Flutter integration_test
# Outputs verdict JSON + logs under local-ci/verification/e2e_proof_pack/mobile_merchant/

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$ROOT"

PROOF_DIR="local-ci/verification/e2e_proof_pack/mobile_merchant"
mkdir -p "$PROOF_DIR"
...
echo "=========================================="
echo "MOBILE MERCHANT INTEGRATION PROOF"
echo "=========================================="
...
# Check if mobile-merchant exists
if [ ! -d "source/apps/mobile-merchant" ]; then
  DETAILS="source/apps/mobile-merchant directory not found."
  echo "BLOCKER: $DETAILS"
  VERDICT="BLOCKED"
else
  echo "mobile-merchant app found. Checking Flutter..."
  ...
  if ! command -v flutter &> /dev/null; then
    ...
    if [ ! -d "integration_test" ]; then
      DETAILS="No integration_test directory found in mobile-merchant."
      cd "$ROOT"
      VERDICT="BLOCKED"
    else
...
echo "=========================================="
echo "Mobile Merchant Integration Proof: $VERDICT"
echo "=========================================="
