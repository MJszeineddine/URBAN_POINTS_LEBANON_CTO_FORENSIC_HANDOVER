...
run_and_capture() {
  ...
  local ec
  set +e
  ( set -o pipefail; "${cmd[@]}" 2>&1 | tee "$out_log" ); ec=${PIPESTATUS[0]}
  set -e
  echo "$ec" > "$exit_file"
...
run_and_capture stub_scan bash -lc "cd '$ROOT_DIR' && grep -R -n -E 'TODO|FIXME|NOT_IMPLEMENTED|throw new Error|placeholder|mock' source tools || true"
if [[ ! -f "$STUB_LOG" ]]; then
  echo -n "" > "$STUB_LOG"
fi
...
python3 - > "$RUN_DIR/stub_analysis.txt" 2>&1 << 'PYCODE' || { log "ERROR: Stub scan summary generation failed"; exit 1; }
import json, os, re
...
cat "$RUN_DIR/stub_analysis.txt" | tee -a "$LOG"
...
cat > "$RUN_DIR/exits.json" << EXITJSON
{
